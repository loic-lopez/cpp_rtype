...
namespace
{
    ...
    void extractScalePositionsAndContentAreaFromTexture(const sf::Texture *const pTexture,
                                                        const sf::IntRect textureRectangle, sf::Vector2f &topLeft,
                                                        sf::Vector2f &bottomRight, sf::Vector2f &contentTopLeft,
                                                        sf::Vector2f &contentBottomRight)
    {
        ...
        for (unsigned int x{1u}; x < static_cast<unsigned int>(textureRectangle.width); ++x)
        {
            if (!foundStart)
            {
                if (image.getPixel(textureRectangle.left + x, textureRectangle.top + textureBottomRightPixel.y) ==
                    sf::Color::Black)
                {
                    foundStart = true;
        ...
        for (unsigned int y{1u}; y < static_cast<unsigned int>(textureRectangle.height); ++y)
        {
            if (!foundStart)
            {
                if (image.getPixel(textureRectangle.left + textureBottomRightPixel.x, textureRectangle.top + y) ==
                    sf::Color::Black)
                {
                    foundStart = true;
...
namespace selbaward
{
    ...
    NinePatch::NinePatch()
            : m_primitiveType(sf::PrimitiveType::Quads), m_vertices(36, sf::Vertex({0.f, 0.f})), m_texture(nullptr),
              m_textureRectangle({0, 0, 3, 3}), m_trimmedSize({0.f, 0.f}), m_size({0.f, 0.f}),
              m_scaleTopLeft({0.f, 0.f}), m_scaleBottomRight({0.f, 0.f}), m_contentTopLeft({0.f, 0.f}),
    ...
    void NinePatch::setTexture(const sf::Texture &texture, const bool resetSize, const bool resetRect)
    {
        ...
        m_trimmedSize = sf::Vector2f{static_cast<float>(m_textureRectangle.width),
                                     static_cast<float>(m_textureRectangle.height)} - trimAmount * 2.f;
        if (resetSize)
            m_size = m_trimmedSize;
        extractScalePositionsAndContentAreaFromTexture(m_texture, m_textureRectangle, m_scaleTopLeft,
                                                       m_scaleBottomRight, m_contentTopLeft, m_contentBottomRight);
        priv_updateVertices();
    }
    ...
    sf::FloatRect NinePatch::getLocalContentArea() const
    {
        ...
        return {topLeft,
                priv_getResultingPositionOfTextureCoord(m_contentBottomRight) - topLeft + sf::Vector2f(1.f, 1.f)};
    }
...
// PRIVATE
    ...
    void NinePatch::priv_updateVerticesTexCoords()
    {
        ...
        const sf::Vector2f textureRectangleOffset{static_cast<float>(m_textureRectangle.left),
                                                  static_cast<float>(m_textureRectangle.top)};
        for (auto &vertex : m_vertices)
