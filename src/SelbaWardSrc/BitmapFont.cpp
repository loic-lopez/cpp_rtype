...
namespace selbaward
{
    ...
    BitmapFont::BitmapFont()
            : m_throwExceptions(true), m_useExternalTexture(false), m_texture(), m_mExternalTexture(nullptr),
              m_numberOfTilesPerRow(1), m_tileSize({0, 0}), m_defaultTextureRect(), m_kernings(),
    ...
    void BitmapFont::setTextureRect(const sf::IntRect &textureRect, const unsigned int glyphIndex)
    {
        if (!priv_isGlyphIndexValid(glyphIndex))
        {
            if (m_throwExceptions)
                throw Exception(
                        exceptionPrefix + "cannot set texture rect - glyph index (" + std::to_string(glyphIndex) +
                        ") out of range.");
            return;
        }
    ...
    void BitmapFont::clearTextureRect(const unsigned int glyphIndex)
    {
        if (!priv_isGlyphIndexValid(glyphIndex))
        {
            if (m_throwExceptions)
                throw Exception(
                        exceptionPrefix + "cannot clear texture rect - glyph index (" + std::to_string(glyphIndex) +
                        ") out of range.");
            return;
        }
    ...
    void BitmapFont::setGlyphToDefault(const unsigned int glyphIndex)
    {
        if (!priv_isGlyphIndexValid(glyphIndex))
        {
            if (m_throwExceptions)
                throw Exception(
                        exceptionPrefix + "cannot set glyph to default - glyph index (" + std::to_string(glyphIndex) +
                        ") out of range.");
            return;
        }
    ...
    void BitmapFont::setBaseline(const int baseline, const unsigned int glyphIndex)
    {
        if (!priv_isGlyphIndexValid(glyphIndex))
        {
            if (m_throwExceptions)
                throw Exception(
                        exceptionPrefix + "cannot set glyph baseline - glyph index (" + std::to_string(glyphIndex) +
                        ") out of range.");
            return;
        }
    ...
    void BitmapFont::setWidth(const int width, const unsigned int glyphIndex)
    {
        if (!priv_isGlyphIndexValid(glyphIndex))
        {
            if (m_throwExceptions)
                throw Exception(
                        exceptionPrefix + "cannot set glyph width - glyph index (" + std::to_string(glyphIndex) +
                        ") out of range.");
            return;
        }
    ...
    void BitmapFont::setStartX(const int startX, const unsigned int glyphIndex)
    {
        if (!priv_isGlyphIndexValid(glyphIndex))
        {
            if (m_throwExceptions)
                throw Exception(
                        exceptionPrefix + "cannot set glyph start x - glyph index (" + std::to_string(glyphIndex) +
                        ") out of range.");
            return;
        }
    ...
    void BitmapFont::setBaselines(const int baseline, const unsigned int numberOfGlyphs,
                                  const unsigned int initialGlyphIndex)
    {
        for (unsigned int i{0}; i < numberOfGlyphs; ++i)
    ...
    void
    BitmapFont::setStartXs(const int startX, const unsigned int numberOfGlyphs, const unsigned int initialGlyphIndex)
    {
        for (unsigned int i{0}; i < numberOfGlyphs; ++i)
...
// PRIVATE
    ...
    const BitmapFont::Glyph BitmapFont::priv_getGlyphWithDefaultTextureRect(unsigned int glyphIndex) const
    {
        if (!priv_isGlyphIndexValid(glyphIndex))
        {
            if (m_throwExceptions)
                throw Exception(
                        exceptionPrefix + "cannot get default glyph - glyph index (" + std::to_string(glyphIndex) +
                        ") out of range.");
            glyphIndex = 0;
        }
