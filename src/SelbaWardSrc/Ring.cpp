...
namespace selbaward
{
    ...
    Ring::Ring(const float radius, const float hole, const unsigned int numberOfSides)
            : m_radius(radius), m_hole(hole), m_sectorSize(1.f), m_sectorOffset(0.f), m_numberOfSides(numberOfSides),
            ...
            , m_vertices((m_numberOfSides + 1) * 2), m_texture(nullptr), m_textureRect()
    {
        priv_updateVertices();
    }
...
// PRIVATE
    ...
    void Ring::priv_updateVertices()
    {
        ...
        for (std::vector<sf::Vertex>::iterator begin{m_vertices.begin()}, end{m_vertices.end()}, it{begin};
             it != end; ++it)
        {
            ...
            it->position = {m_radius + std::sin(angle) * m_radius * (isInnerPoint ? m_hole : 1.f),
                            m_radius - std::cos(angle) * m_radius * (isInnerPoint ? m_hole : 1.f),};
            it->color = m_color;
            const sf::Vector2f scaledPosition{it->position / (m_radius * 2.f)};
            it->texCoords = {scaledPosition.x * m_textureRect.width + m_textureRect.left,
