...
namespace selbaward
{
    ...
    PieChart::Slice::Slice()
            : size(0.1f), scale(1.f), explode(0.f), color(sf::Color::White)
    {
    }

    PieChart::PieChart()
            : m_primitive(sf::PrimitiveType::Triangles), m_vertices(), m_size({32.f, 32.f})
    {
    }
    ...
    void PieChart::update()
    {
        ...
        for (unsigned int slice{0u}; slice < slices.size(); ++slice)
        {
            ...
            const sf::Vector2f offset{sf::Vector2f(sin(radiansFromDegrees(startAngle + halfAngleDifference)),
                                                   -cos(radiansFromDegrees(startAngle + halfAngleDifference))) *
                                      slices[slice].explode};
            for (unsigned int triangle{0u}; triangle < numberOfTrianglesRequiredPerSlice[slice]; ++triangle)
            {
                m_vertices[currentVertex + 0].position =
                        halfSize + sf::Vector2f(offset.x * halfSize.x, offset.y * halfSize.y);
                m_vertices[currentVertex + 1].position = halfSize + sf::Vector2f(
                        (offset.x + sin(radiansFromDegrees(currentAngle))) * halfSize.x,
                        (offset.y - cos(radiansFromDegrees(currentAngle))) * halfSize.y) * slices[slice].scale;
                currentAngle += halfAngleDifference * 2.f / numberOfTrianglesRequiredPerSlice[slice];
                m_vertices[currentVertex + 2].position = halfSize + sf::Vector2f(
                        (offset.x + sin(radiansFromDegrees(currentAngle))) * halfSize.x,
                        (offset.y - cos(radiansFromDegrees(currentAngle))) * halfSize.y) * slices[slice].scale;
