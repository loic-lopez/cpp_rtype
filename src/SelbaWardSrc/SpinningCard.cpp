...
namespace selbaward
{
...
#endif // USE_SFML_PRE_2_4
    {
        ...
        m_vertices[0].position += sf::Vector2f(static_cast<float>(m_initial.width) / 2,
                                               static_cast<float>(m_initial.height) / 2); // centre
        ...
        for (unsigned int i{0}; i < 5; ++i)
            m_vertices[i].texCoords = sf::Vector2f(static_cast<float>(sprite.getTextureRect().left),
                                                   static_cast<float>(sprite.getTextureRect().top)); // top-left
        m_vertices[0].texCoords += sf::Vector2f(static_cast<float>(sprite.getTextureRect().width) / 2,
                                                static_cast<float>(sprite.getTextureRect().height) / 2); // centre
        m_vertices[2].texCoords += sf::Vector2f(static_cast<float>(sprite.getTextureRect().width), 0.f); // top-right
        m_vertices[3].texCoords += sf::Vector2f(static_cast<float>(sprite.getTextureRect().width),
                                                static_cast<float>(sprite.getTextureRect().height)); // bottom-right
    ...
    void SpinningCard::spinRadians(float angleInRadians)
    {
        ...
        m_vertices[0].position = sf::Vector2f(m_initial.left + m_initial.width / 2,
                                              m_initial.top + m_initial.height / 2);
        m_vertices[1].position = sf::Vector2f(m_initial.left + (sin(angleInRadians) + 1) * m_initial.width / 2,
                                              m_initial.top - cos(angleInRadians) * m_depth * m_initial.height / 2);
        m_vertices[2].position = sf::Vector2f(m_initial.left + (sin(angleInRadians + m_pi) + 1) * m_initial.width / 2,
                                              m_initial.top -
                                              cos(angleInRadians + m_pi) * m_depth * m_initial.height / 2);
        m_vertices[3].position = sf::Vector2f(m_initial.left + (sin(angleInRadians + m_pi) + 1) * m_initial.width / 2,
                                              m_initial.top +
                                              (cos(angleInRadians + m_pi) * m_depth + 2) * m_initial.height / 2);
        m_vertices[4].position = sf::Vector2f(m_initial.left + (sin(angleInRadians) + 1) * m_initial.width / 2,
                                              m_initial.top +
                                              (cos(angleInRadians) * m_depth + 2) * m_initial.height / 2);
    ...
    void SpinningCard::spinVerticallyRadians(float angleInRadians)
    {
        ...
        m_vertices[0].position = sf::Vector2f(m_initial.left + m_initial.width / 2,
                                              m_initial.top + m_initial.height / 2);
        m_vertices[1].position = sf::Vector2f(m_initial.left - cos(angleInRadians) * m_depth * m_initial.width / 2,
                                              m_initial.top + (sin(angleInRadians) + 1) * m_initial.height / 2);
        m_vertices[4].position = sf::Vector2f(
                m_initial.left - cos(angleInRadians + m_pi) * m_depth * m_initial.width / 2,
                m_initial.top + (sin(angleInRadians + m_pi) + 1) * m_initial.height / 2);
        m_vertices[3].position = sf::Vector2f(
                m_initial.left + (cos(angleInRadians + m_pi) * m_depth + 2) * m_initial.width / 2,
                m_initial.top + (sin(angleInRadians + m_pi) + 1) * m_initial.height / 2);
        m_vertices[2].position = sf::Vector2f(
                m_initial.left + (cos(angleInRadians) * m_depth + 2) * m_initial.width / 2,
                m_initial.top + (sin(angleInRadians) + 1) * m_initial.height / 2);
