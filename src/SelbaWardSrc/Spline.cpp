...
namespace
{
    ...
    {
        return sf::Color(linearInterpolation(start.r, end.r, alpha), linearInterpolation(start.g, end.g, alpha),
                         linearInterpolation(start.b, end.b, alpha));
    }

    inline sf::Vector2f
    bezierInterpolation(const sf::Vector2f start, const sf::Vector2f end, const sf::Vector2f startHandle,
                        const sf::Vector2f endHandle, const float alpha)
    {
        ...
        return
                {
                        start.x * alpha2Squared * alpha2 + startControl.x * 3 * alpha2Squared * alpha +
                        endControl.x * 3 * alpha2 * alphaSquared + end.x * alpha * alphaSquared,
                        start.y * alpha2Squared * alpha2 + startControl.y * 3 * alpha2Squared * alpha +
                        endControl.y * 3 * alpha2 * alphaSquared + end.y * alpha * alphaSquared
                };
    }
...
namespace selbaward
{
    ...
    Spline::Spline(const unsigned int vertexCount, const sf::Vector2f initialPosition)
            : m_throwExceptions(true), m_isClosed(false), m_vertices(vertexCount, Vertex(initialPosition)),
              m_color(sf::Color::White), m_thickness(0.f), m_sfmlVertices(), m_sfmlThickVertices()
#ifdef USE_SFML_PRE_2_4
            , m_primitiveType(sf::PrimitiveType::LinesStrip)
#else // USE_SFML_PRE_2_4
            , m_primitiveType(sf::PrimitiveType::LineStrip)
#endif // USE_SFML_PRE_2_4
            , m_interpolationSteps(0u), m_handlesVertices(), m_showHandles(false), m_useBezier(false),
              m_lockHandleMirror(true), m_lockHandleAngle(true)
    {
    }
    ...
    void Spline::update()
    {
            ...
            if (m_isClosed || it != end - 1)
            {
                for (unsigned int i{0u}; i < pointsPerVertex; ++i)
                {
                    ...
                    if (it != end - 1)
                        nextIt = it + 1;
                    if (m_useBezier)
                        itSfml->position = bezierInterpolation(it->position, nextIt->position, it->frontHandle,
                                                               nextIt->backHandle,
                                                               static_cast<float>(i) / pointsPerVertex);
                    else
                        itSfml->position = linearInterpolation(it->position, nextIt->position,
                                                               static_cast<float>(i) / pointsPerVertex);
        ...
        // calculate tangents
        for (std::vector<sf::Vertex>::iterator begin{m_sfmlVertices.begin()}, end{m_sfmlVertices.end()}, current{begin};
             current != end; ++current)
        {
        ...
        {
            m_sfmlThickVertices.resize(m_sfmlVertices.size() *
                                       2); // required number of vertices for a triangle strip (two triangles between each pair of points)

            std::vector<sf::Vertex>::iterator itThick{m_sfmlThickVertices.begin()};
            for (std::vector<sf::Vertex>::iterator begin{m_sfmlVertices.begin()}, end{m_sfmlVertices.end()}, it{begin};
                 it != end; ++it)
            {
                ...
                const float vertexRatio{
                        static_cast<float>(sfmlIndex - interpolatedIndex) / priv_getNumberOfPointsPerVertex()};
                ...
                const float thickness{m_thickness * linearInterpolation(currentVertex->thickness, nextVertex->thickness,
                                                                        vertexRatio)};
                ...
                if (m_isClosed || it != end - 1)
                {
                    const sf::Vector2f forwardLine{
                            ((it != end - 1) ? (it + 1)->position - it->position : (begin + 1)->position -
                                                                                   begin->position)};
    ...
    {
        if (!priv_testVertexIndex(index, "Cannot remove vertices") ||
            ((numberOfVertices > 1) && (!priv_testVertexIndex(index + numberOfVertices - 1, "Cannot remove vertices"))))
            return;
    ...
    {
        if (!priv_testVertexIndex(index, "Cannot set vertices' positions") || (numberOfVertices > 1 &&
                                                                               !priv_testVertexIndex(
                                                                                       index + numberOfVertices - 1,
                                                                                       "Cannot set vertices' positions")))
            return;
    ...
    void Spline::setPositions(const std::vector<sf::Vector2f> &positions, unsigned int index)
    {
        ...
        if ((numberOfVertices < 1) || (!priv_testVertexIndex(index, "Cannot set vertices' positions")) ||
            ((numberOfVertices > 1) &&
             (!priv_testVertexIndex(index + numberOfVertices - 1, "Cannot set vertices' positions"))))
            return;
    ...
    {
        if ((!priv_testVertexIndex(index, "Cannot reset vertices' handles")) || ((numberOfVertices > 1) &&
                                                                                 (!priv_testVertexIndex(
                                                                                         index + numberOfVertices - 1,
                                                                                         "Cannot reset vertices' handles"))))
            return;
    ...
    {
        return ((m_isClosed) ? (static_cast<unsigned int>(m_vertices.size()) * priv_getNumberOfPointsPerVertex()) : (
                (static_cast<unsigned int>(m_vertices.size()) - 1u) * priv_getNumberOfPointsPerVertex() + 1u));
    }

    sf::Vector2f
    Spline::getInterpolatedPositionTangent(const unsigned int interpolationOffset, const unsigned int index) const
    {
        return m_sfmlVerticesUnitTangents[priv_getInterpolatedIndex(interpolationOffset, index)];
    }

    sf::Vector2f
    Spline::getInterpolatedPositionNormal(const unsigned int interpolationOffset, const unsigned int index) const
    {
        return vectorNormal(getInterpolatedPositionTangent(interpolationOffset, index));
    }

    float
    Spline::getInterpolatedPositionThickness(const unsigned int interpolationOffset, const unsigned int index) const
    {
        if (!priv_isThick())
            return 0.f;
        ...
        const float vertexRatio{
                static_cast<float>(interpolatedIndex - interpolatedVertexIndex) / priv_getNumberOfPointsPerVertex()};
    ...
    float Spline::getInterpolatedPositionThicknessCorrectionScale(const unsigned int interpolationOffset,
                                                                  const unsigned int index) const
    {
        ...
        const sf::Vector2f sideOffset{
                m_sfmlThickVertices[interpolatedIndex * 2].position - m_sfmlVertices[interpolatedIndex].position};
    ...
    bool Spline::priv_testVertexIndex(const unsigned int vertexIndex, const std::string &exceptionMessage) const
    {
        if (!priv_isValidVertexIndex(vertexIndex))
        {
            if (m_throwExceptions)
                throw Exception(exceptionPrefix + exceptionMessage + " Vertex index (" + std::to_string(vertexIndex) +
                                ") out of range");
            return false;
        }
        return true;
...
// index and interpolationOffset values must be valid
    unsigned int
    Spline::priv_getInterpolatedIndex(const unsigned int interpolationOffset, const unsigned int index) const
    {
