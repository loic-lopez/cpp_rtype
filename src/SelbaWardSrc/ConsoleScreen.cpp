...
namespace
{
    ...
    const selbaward::ConsoleScreen::Cell defaultCell{0u, selbaward::ConsoleScreen::ColorPair(),
                                                     selbaward::ConsoleScreen::StretchType::None,
...
namespace selbaward
{
    ...
    ConsoleScreen &ConsoleScreen::operator<<(const CursorCommand &cursorCommand)
    {
        ...
        switch (cursorCommand)
        {
            ...
            case CursorCommand::Backspace:
                ...
                if (printProperties.stretch == StretchType::Both && printProperties.index >= m_mode.x &&
                    printProperties.index % m_mode.x == 0)
                    printProperties.index -= m_mode.x;
                if (printProperties.index > 0)
                    --printProperties.index;
    ...
    ConsoleScreen::ConsoleScreen(const sf::Vector2u mode)
            : m_cells(), m_mode(mode), m_buffers(), m_do(), m_is(), m_display(), m_backgroundDisplay(), m_overDisplay(),
              m_underDisplay(), m_size{100.f, 100.f}, m_texture(nullptr), m_textureOffset{0u, 0u}, m_tileSize{8u, 8u},
              m_numberOfTilesPerRow{16u}, m_palette(),
              m_cursor{static_cast<int>('_'), defaultCursorColor, true, false, false}, m_tabSize{4u}, m_readLength{1u},
              m_clearValue{0u}, m_darkAttributeMultiplier{0.5f}, m_characterMap(),
              m_defaultPrintProperties{0u, ColorPair(defaultColor, defaultBackgroundColor), StretchType::None,
                                       CellAttributes(), Affect::Default, ColorType::Foreground}
    {
    ...
    void ConsoleScreen::clearOversAt(const Location &location)
    {
        ...
        { return stackCell.index == index; }), m_overCells.end());
    ...
    void ConsoleScreen::clearUndersAt(const Location &location)
    {
        ...
        { return stackCell.index == index; }), m_underCells.end());
        ...
        if (!priv_isCellLocationInRange(location))
        {
            if (m_do.throwExceptions)
                throw Exception(exceptionPrefix + "Cannot set cell background colors.\nLocation (" +
                                std::to_string(location.x) + ", " + std::to_string(location.y) + ") out of range.");
            return;
        }
                ...
                for (unsigned int x{0}; x < static_cast<unsigned int>(selectionRectangle.width); ++x)
                {
                    ...
                    if (y < static_cast<unsigned int>(selectionRectangle.height) - 1)
                        m_cells[priv_cellIndex(
                                {selectionRectangle.left + x, selectionRectangle.top + y})] = m_cells[priv_cellIndex(
                                {selectionRectangle.left + x, selectionRectangle.top + y + 1})];
                    else if (m_do.wrapOnManualScroll)
            ...
            for (unsigned int x{0}; x < static_cast<unsigned int>(selectionRectangle.width); ++x)
            {
                for (unsigned int y{0}; y < static_cast<unsigned int>(selectionRectangle.height); ++y)
                {
                    ...
                    if (x < static_cast<unsigned int>(selectionRectangle.width) - 1)
                        m_cells[priv_cellIndex(
                                {selectionRectangle.left + x, selectionRectangle.top + y})] = m_cells[priv_cellIndex(
                                {selectionRectangle.left + x + 1, selectionRectangle.top + y})];
                    else if (m_do.wrapOnManualScroll)
                        m_cells[priv_cellIndex(
                                {selectionRectangle.left + x, selectionRectangle.top + y})] = leftColumn[y];
                    else
                        priv_clearCell(priv_cellIndex({selectionRectangle.left + x, selectionRectangle.top + y}), true,
                                       true);
                }
    ...
    sf::Color ConsoleScreen::getPaletteColor(const Color color) const
    {
        ...
        return m_is.rgbMode ? sf::Color(static_cast<sf::Uint8>(color.id / 65536),
                                        static_cast<sf::Uint8>((color.id % 65536) / 256),
                                        static_cast<sf::Uint8>(color.id % 256)) : m_palette[color.id];
    }
...
// PRIVATE
    ...
    void ConsoleScreen::priv_setVerticesFromCell(unsigned int index, int baseVertex, const bool overLayer)
    {
        ...
        if (mainLayer)
        {
            ...
            backgroundColor = getPaletteColor(
                    priv_getModifiedColorFromCellUsingSpecifiedColorType(index, ColorType::Background).id);
        }
        else
        {
        ...
        const bool isCursorBottom{(m_cursor.visible) && (m_cursorPrintProperties.stretch == Cs::StretchType::Both) &&
                                  (m_cursorPrintProperties.index + m_mode.x == index)};
        ...
        const float left{offset.x + linearInterpolation(0.f, m_size.x,
                                                        static_cast<float>(cellX + (cell.attributes.flipX ? 1 : 0)) /
                                                        m_mode.x)};
        const float right{offset.x + linearInterpolation(0.f, m_size.x,
                                                         static_cast<float>(cellX + (cell.attributes.flipX ? 0 : 1)) /
                                                         m_mode.x)};
        const float top{offset.y + linearInterpolation(0.f, m_size.y,
                                                       static_cast<float>(cellY + (cell.attributes.flipY ? 1 : 0)) /
                                                       m_mode.y)};
        const float bottom{offset.y + linearInterpolation(0.f, m_size.y,
                                                          static_cast<float>(cellY + (cell.attributes.flipY ? 0 : 1)) /
                                                          m_mode.y)};
        ...
        // main display
        sf::Vertex *const pVertex1{(mainLayer ? &m_display[baseVertex] : overLayer ? &m_overDisplay[baseVertex]
                                                                                   : &m_underDisplay[baseVertex])};
    ...
    {
        return (color.id >= 0) &&
               ((m_is.rgbMode && color.id < 16777216L) || (color.id < static_cast<long int>(m_palette.size())));
    }
                                       ...
                                       const bool overwriteBackgroundColor)
    {
        ...
        const Color backgroundColor{overwriteBackgroundColor ? m_cursorPrintProperties.colors.background
                                                             : m_cells[index].colors.background};
        poke(index, {m_clearValue, ColorPair(color, backgroundColor), StretchType::None, CellAttributes()});
    }

    void
    ConsoleScreen::priv_clearCell(const unsigned int index, const Color &backgroundColor, const bool overwriteColor)
    {
        const Color color{
                overwriteColor ? m_cursorPrintProperties.colors.foreground : m_cells[index].colors.foreground};
        poke(index, {m_clearValue, ColorPair(color, backgroundColor), StretchType::None, CellAttributes()});
    ...
    {
        return Color(std::uniform_int_distribution<unsigned long int>(0u, m_is.rgbMode ? 16777215L :
                                                                          static_cast<unsigned long int>(m_palette.size()) -
                                                                          1u)(randomGenerator));
    }
    ...
    ConsoleScreen::Color
    ConsoleScreen::priv_getModifiedColorFromColorPairUsingSpecifiedColorType(const ColorPair &colors,
    ...
    ConsoleScreen::Color
    ConsoleScreen::priv_getModifiedColorFromCellUsingSpecifiedColorType(const unsigned int &cellIndex,
                                                                        const ColorType &colorType)
    {
    ...
    {
        PrintProperties &printProperties{priv_getCurrentPrintProperties()};
        return (printProperties.colorType == ColorType::Foreground) ? printProperties.colors.foreground
    ...
    {
        PrintProperties &printProperties{priv_getCurrentPrintProperties()};
        return (printProperties.colorType == ColorType::Foreground) ? printProperties.colors.background
