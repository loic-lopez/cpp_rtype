...
namespace selbaward
{
    ...
    GallerySprite::GallerySprite()
            : m_vertices(4), m_pTexture(nullptr), m_exhibits(), m_currentExhibit(0u)
    {
        priv_updateVertices();
    }
...
// PRIVATE
    ...
    void GallerySprite::priv_updateVertices()
    {
        ...
        m_vertices[2].position = {exhibit.rectangle.width - exhibit.anchor.x,
                                  exhibit.rectangle.height - exhibit.anchor.y};
        ...
        m_vertices[2].texCoords = {exhibit.rectangle.left + exhibit.rectangle.width,
                                   exhibit.rectangle.top + exhibit.rectangle.height};
    ...
    GallerySprite::Exhibit GallerySprite::priv_getExhibit(unsigned int exhibitNumber) const
    {
        ...
        if (exhibitNumber == 0u)
            return {{0.f, 0.f, static_cast<float>(m_pTexture->getSize().x), static_cast<float>(m_pTexture->getSize().y)},
                    {0.f, 0.f}};
