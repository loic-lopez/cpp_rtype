...
namespace selbaward
{
    ...
    Sprite3d::Sprite3d()
            : m_depthToShallownessConversionNumerator(10000.f), m_pitch(0.f), m_yaw(0.f), m_depth(10.f),
              m_shallowness(m_depthToShallownessConversionNumerator / m_depth), m_meshDensity(0u), m_flipBack(false),
              m_pTexture(nullptr), m_pBackTexture(nullptr), m_textureOffset(), m_backTextureOffset(), m_size(),
              m_useDynamicSubdivision(false), m_minSubdivision(1u), m_maxSubdivision(4u), m_subdivision(0u),
              m_subdividedMeshDensity(0u), m_points(4), m_transformedPoints(4), m_origin(), m_vertices(4),
    ...
    Sprite3d::Sprite3d(const sf::Texture &texture, const sf::IntRect &textureRect, const sf::Texture &backTexture,
                       const sf::Vector2i backTextureOffset)
            : Sprite3d()
    {
        setTexture(texture);
        setTextureRect(textureRect);
    ...
    {
        m_depth = depth;
        m_shallowness = m_depthToShallownessConversionNumerator /
                        ((m_depth > -0.000001f && m_depth < 0.000001f) ? 0.000001f
                                                                       : m_depth); // avoid division by zero here but don't change m_depth from being zero
    }
    ...
    void Sprite3d::updateTransformedPoints() const
    {
        if (m_useDynamicSubdivision)
            setSubdivision(
                    static_cast<unsigned int>((m_maxSubdivision - m_minSubdivision) * getMostExtremeAngle() / 90.f +
                                              m_minSubdivision));
        ...
        *******************************************************/
        ...
        m_compactTransformMatrix = {cosYaw, sinYaw, sinPitch * sinYaw, cosPitch, -sinPitch * cosYaw, -cosPitch * sinYaw,
                                    sinPitch, cosPitch * cosYaw}; // all eight used elements
    ...
    void Sprite3d::updateVertices() const
    {
        ...
        for (unsigned int v{0u}; v < m_vertices.size(); ++v)
        {
            ...
            m_vertices[v].texCoords.x =
                    (m_points[texturePointIndex].x * (m_size.x < 0 ? -1 : 1)) + currentTextureOffset.x;
            m_vertices[v].texCoords.y =
                    (m_points[texturePointIndex].y * (m_size.y < 0 ? -1 : 1)) + currentTextureOffset.y;
        }
    }
    ...
    void Sprite3d::updateGlobalCorners() const
    {
        ...
        m_bottomLeft = getTransform().transformPoint(*(m_transformedPoints.end() - m_subdividedMeshDensity -
                                                       2)); // end() - (m_subdividedMeshDensity + 1) - 1
    ...
    void Sprite3d::createPointGrid() const
    {
        ...
        for (unsigned int y{0u}; y < numberOfPointsPerDimension; ++y)
        {
            ...
            {
                m_points[y * numberOfPointsPerDimension + x].x = linearInterpolation(leftTop.x, rightBottom.x,
                                                                                     static_cast<float>(x) /
                                                                                     (numberOfPointsPerDimension - 1));
                m_points[y * numberOfPointsPerDimension + x].y = linearInterpolation(leftTop.y, rightBottom.y,
                                                                                     static_cast<float>(y) /
                                                                                     (numberOfPointsPerDimension - 1));
