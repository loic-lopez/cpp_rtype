...
namespace selbaward
{
    ...
    TileMap::TileMap()
            : m_primitiveType(sf::PrimitiveType::Quads), m_vertices(), m_size(), m_grid(1u), m_gridSize({1u, 1u}),
              m_outOfBoundsTile(0u), m_texture(nullptr), m_numberOfTextureTilesPerRow(16u), m_textureOffset({0u, 0u}),
              m_textureTileSize({16u, 16u}), m_renderTexture(), m_render(4), m_is(), m_do(), m_camera({0.f, 0.f}),
              m_cameraTarget({0.f, 0.f}), m_color(sf::Color::White), m_redrawRequired(true)
    {
    ...
    sf::Vector2i TileMap::getLevelPositionAtCoord(sf::Vector2f coord) const
    {
        ...
        return {static_cast<int>(floor(coord.x * (m_gridSize.x - 1) / m_size.x + actualCamera.x)),
                static_cast<int>(floor(coord.y * (m_gridSize.y - 1) / m_size.y + actualCamera.y))};
    }
    ...
    {
        const sf::Vector2f actualCamera{priv_getActualCamera()};
        const sf::Vector2f local{(levelGridPosition.x - actualCamera.x) * m_size.x / (m_gridSize.x - 1),
                                 (levelGridPosition.y - actualCamera.y) * m_size.y / (m_gridSize.y - 1)};
...
// PRIVATE
    ...
    void TileMap::priv_updateVertices() const
    {
        ...
        for (unsigned int y{0}; y < m_gridSize.y; ++y)
        {
            for (unsigned int x{0}; x < m_gridSize.x; ++x)
            {
                ...
                const sf::Vector2u textureTilePosition{
                        static_cast<unsigned int>(tileValue % m_numberOfTextureTilesPerRow * m_textureTileSize.x),
                        static_cast<unsigned int>(tileValue / m_numberOfTextureTilesPerRow * m_textureTileSize.y)};
                sf::Vertex *pVertex{&m_vertices[tileIndex * 4]};

                // top-left
                pVertex->position = {static_cast<float>(m_textureTileSize.x * x),
                                     static_cast<float>(m_textureTileSize.y * y)};
                pVertex->texCoords = sf::Vector2f(m_textureOffset + textureTilePosition);
                pVertex++->color = m_color;

                // top-right
                pVertex->position = {static_cast<float>(m_textureTileSize.x * (x + 1)),
                                     static_cast<float>(m_textureTileSize.y * y)};
                pVertex->texCoords = {
                        static_cast<float>(m_textureOffset.x + textureTilePosition.x + m_textureTileSize.x),
                        static_cast<float>(m_textureOffset.y + textureTilePosition.y)};
                pVertex++->color = m_color;

                // bottom-right
                pVertex->position = {static_cast<float>(m_textureTileSize.x * (x + 1)),
                                     static_cast<float>(m_textureTileSize.y * (y + 1))};
                ...
                pVertex->position = {static_cast<float>(m_textureTileSize.x * x),
                                     static_cast<float>(m_textureTileSize.y * (y + 1))};
                pVertex->texCoords = {static_cast<float>(m_textureOffset.x + textureTilePosition.x),
                                      static_cast<float>(m_textureOffset.y + textureTilePosition.y +
                                                         m_textureTileSize.y)};
                pVertex++->color = m_color;
            }
        ...
        const sf::Vector2f actualCamera{priv_getActualCamera()};
        const sf::Vector2f cameraOffset{trunc((actualCamera.x - floor(actualCamera.x)) * m_textureTileSize.x),
                                        trunc((actualCamera.y - floor(actualCamera.y)) * m_textureTileSize.y)};
    ...
    void TileMap::priv_updateRender() const
    {
        ...
        if (m_do.scrollSmoothly)
        {
            ...
            const sf::Vector2f actualCamera{priv_getActualCamera()};
            const sf::Vector2f cameraOffset{((actualCamera.x - floor(actualCamera.x)) * m_textureTileSize.x),
                                            ((actualCamera.y - floor(actualCamera.y)) * m_textureTileSize.y)};
            const sf::Vector2f fractionOffset{round((cameraOffset.x - floor(cameraOffset.x)) * pixelSize.x),
                                              round((cameraOffset.y - floor(cameraOffset.y)) * pixelSize.y)};
    ...
    sf::Vector2i TileMap::priv_getGridPositionAtCoord(sf::Vector2f coord) const
    {
        ...
        return {static_cast<int>(floor(coord.x * (m_gridSize.x - 1) / m_size.x + fraction.x)),
                static_cast<int>(floor(coord.y * (m_gridSize.y - 1) / m_size.y + fraction.y))};
    }
