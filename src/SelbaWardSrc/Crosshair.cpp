...
namespace
{
    ...
    bool
    updateVertices(sf::VertexArray &vertices, const sf::RenderWindow *const window, const sf::Color &horizontalColor,
                   const sf::Color &verticalColor)
    {
        if (window == nullptr)
            return false;
...
namespace selbaward
{
    ...
    Crosshair::Crosshair(const sf::Color &horizontalColor, const sf::Color &verticalColor)
            : m_vertices(sf::PrimitiveType::Lines, 4), m_horizontalColor(horizontalColor),
              m_verticalColor(verticalColor), m_window(nullptr)
    {
    }
