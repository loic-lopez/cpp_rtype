...
namespace selbaward
{
    ...
    BitmapText::BitmapText()
            : m_pBitmapFont(nullptr), m_vertices(sf::Quads), m_string(), m_color(sf::Color::White), m_tracking(1)
    {
    }
...
// PRIVATE
    ...
    void BitmapText::priv_updateVertices()
    {
        ...
        {
            const unsigned int glyphNumber{(m_string[character] >= 0) ? static_cast<unsigned int>(m_string[character])
                                                                      : static_cast<unsigned int>(m_string[character] +
                                                                                                  256)}; // after 125, 126, 127 is -128, -127, -126. this moves them to 128, 129, 130

            const int kerning{
                    (character < m_string.length() - 1) ? m_pBitmapFont->getKerning(m_string.substr(character, 2)) : 0};

            BitmapFont::Glyph glyph = m_pBitmapFont->getGlyph(glyphNumber);

            const sf::Vector2f glyphOffset{0.f - glyph.startX,
                                           (glyph.baseline < 0) ? (0.f - glyph.baseline - glyph.textureRect.height) : (
                                                   0.f - glyph.baseline)};
            const sf::Vector2f glyphPosition{penPosition + glyphOffset};

            m_vertices[character * 4 + 0].position = glyphPosition;
            m_vertices[character * 4 + 1].position =
                    glyphPosition + sf::Vector2f(static_cast<float>(glyph.textureRect.width), 0);
            m_vertices[character * 4 + 2].position = glyphPosition +
                                                     sf::Vector2f(static_cast<float>(glyph.textureRect.width),
                                                                  static_cast<float>(glyph.textureRect.height));
            m_vertices[character * 4 + 3].position =
                    glyphPosition + sf::Vector2f(0, static_cast<float>(glyph.textureRect.height));
            ...
            penPosition += sf::Vector2f(
                    (glyph.width > 0) ? (0.f + m_tracking + kerning + glyph.width) : (0.f + m_tracking + kerning +
                                                                                      glyph.width +
